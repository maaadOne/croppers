...
import { ProcessOptions, UploadReady } from './images.interface';

type Fmt = 'webp' | 'jpeg';
// для возврата корректного MIME-типа
function contentType(fmt: Fmt): string {
...
export class ImagesProcessor {
	 ...
	 * - обновление в кэше
	 */
	async process(opts: ProcessOptions): Promise<UploadReady> {
		...
		try {
			...
			const quality = Math.max(
				1,
				Math.min(100, Math.floor(Number(crop.quality ?? 82))),
			);
			...
			img =
				fmt === 'webp' ? img.webp({ quality }) : img.jpeg({ quality });
