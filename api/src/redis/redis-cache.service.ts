...
export class RedisCacheService implements OnModuleDestroy {
  ...
  async addKnownHash(hash: string) { await this.redis.sadd('img:known', hash); }
  async isKnownHash(hash: string) { return (await this.redis.sismember('img:known', hash)) === 1; }
  ...
  async unlock(key: string) { await this.redis.del(key); }

  // Pub/Sub (простая публикация)
  async publish(channel: string, message: string) { await this.redis.publish(channel, message); }
}
